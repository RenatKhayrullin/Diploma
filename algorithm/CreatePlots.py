    ...
    for seed_size, cluster_size_values in loss_graphics_data.items():
        for clusters_count, loss_valies in cluster_size_values.items():
            if clusters_count not in errors_dict:
                ...
                errors_dict[clusters_count]["iterations"] = iter_counts
...
def create_score_plots(df, filename):
    plt.plot(df['seed_size'], df['precision'], marker='', color='blue', linewidth=2, label="Precision")
    plt.plot(df['seed_size'], df['recall'], marker='', color='green', linewidth=2, label="Recall")
    plt.plot(df['seed_size'], df['f1'], marker='', color='orange', linewidth=2, label="F1")
    ...
    for seed_size, cluster_size_scores in score_graphics_data.items():
        for clusters_count, score_valies in cluster_size_scores.items():
            ...
            for class_id, class_scores in score_valies.items():
                ...
                score_dict[clusters_count][class_id]["precision"].append(float(score_valies[class_id]["precision"]))
                score_dict[clusters_count][class_id]["recall"].append(float(score_valies[class_id]["recall"]))
                score_dict[clusters_count][class_id]["f1"].append(float(score_valies[class_id]["f1"]))

    for clusters_count in score_dict.keys():
        for class_id in score_dict[clusters_count].keys():
            graphic_file_name = filename + "_clusters-" + clusters_count+"_"+class_id
            ...
            create_score_plots(df.sort_values(['seed_size']), graphic_file_name)
...
def create_plots():
        ...
        for file in files:
            ...
            with io.open(data_file, 'r', encoding='utf8') as r_file:
                ...
                if file_type == "score":
                    ...
                    if algorithm_type not in score_graphics:
                        score_graphics[algorithm_type] = {}
                    if seed_size not in score_graphics[algorithm_type]:
    ...
    draw_score_plots(score_graphics["NOCLUS"], "NOCLUS_score")
    draw_score_plots(score_graphics["CLUS"], "CLUS_score")
